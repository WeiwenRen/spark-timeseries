...
private[sparkts] object DateTimeIndexUtils {
  implicit val dateTimeIndexOrdering: Ordering[DateTimeIndex] = new Ordering[DateTimeIndex] {
  ...
  : DateTimeIndex = {
    val indicesPQ = mutable.PriorityQueue.empty[DateTimeIndex](dateTimeIndexOrdering.reverse)
    indices.foreach(indicesPQ.enqueue(_))

    val unionList = new ListBuffer[DateTimeIndex]
    unionList += indicesPQ.dequeue

    while (indicesPQ.nonEmpty) {
